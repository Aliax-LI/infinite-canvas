export default {
    ...
    prompts: {
        ...
        clearFilters: "清除筛选",
        searching: "正在搜索，首次打开较大的来源时可能稍慢",
        loadingCatalog: "正在加载全部来源…",
    ...
    imageWorkbench: {
        ...
        empty: "还没有生成图片",
        emptyHint: "在底部描述画面，回车即可生成。参考图、提示词库和参数都在输入栏里。",
        fillComposer: "填入输入栏",
    ...
    config: {
        ...
        sub2apiImporting: "正在同步 Sub2API 渠道…",
        sub2apiImportFailed: "Sub2API 密钥创建或渠道导入失败",
        sub2apiExistingKeyNoSecret: "Sub2API 已有「无限画布」密钥，但无法读取明文；请在本机曾导入过的浏览器打开，或在设置中手动填写 API Key",
        sub2apiGroupPickerTitle: "选择 Sub2API 分组",
        sub2apiGroupPickerDescription: "首次嵌入需要创建「无限画布」API Key，请选择该密钥所属分组。",
        sub2apiGroupPickerAssignDescription: "「无限画布」API Key 尚未绑定分组，请选择分组后再继续导入。",
        sub2apiGroupPickerAssignConfirm: "绑定并导入",
        sub2apiGroupsLoadFailed: "无法获取 Sub2API 可选分组，请从 Sub2API 重新打开嵌入链接，或在链接中带上 group_id",
        sub2apiGroupPickerPlaceholder: "请选择分组",
        sub2apiGroupPickerConfirm: "创建并导入",
        sub2apiGroupRequired: "请先选择分组",
        sub2apiGroupFallbackName: "分组 #{{id}}",
